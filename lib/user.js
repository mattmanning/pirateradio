...
function build_user(data) {
  ...
  log('user.build_user', { id:data.id, identity:data.identity });
...
module.exports.lookup = function(id, callback) {
  ...
  module.exports.database().getDoc(id, function(error, user) {
    log('user.lookup.get', { query:id, id:(user && user.id) })
    callback(build_user(user));
  });
}

module.exports.lookup_by_identity = function(identity, callback) {
  log('user.lookup_by_identity', { identity:identity });

  module.exports.database().view('users', 'by_identity', {
    limit:1, 
    startkey:identity,
    endkey:identity+"\u0000"
  }, function(error, data) {
    var user = data.rows[0] && data.rows[0].value;
    log('user.lookup_by_identity.get', { identity:(user && user.identity) })
...
module.exports.middleware = function(settings) {
  ...
  return function(request, response, next) {
    module.exports.lookup_by_identity(request.identity, function(user) {
      log('user.middleware.call', { identity:request.identity });
      request.user = user;
