...
var Astrolabe = function(settings) {
  ...
  this.update = function(id, position) {
    ...
    postgres.query("insert into locations (id, radius, located_at, location) values (" +
      ...
      function(error) {
        console.log("SELECT l2.id FROM locations AS l1 INNER JOIN locations AS l2 ON ST_Distance(l1.location, l2.location) < l1.radius WHERE l1.id = '" + id + "';");
        postgres.query("SELECT l2.id FROM locations AS l1 INNER JOIN locations AS l2 ON ST_Distance(l1.location, l2.location) < l1.radius WHERE l1.id = '" + id + "';", function(error, rows) {
          if (rows) {
        ...
        console.log("SELECT l2.id FROM locations AS l1 INNER JOIN locations AS l2 ON ST_Distance(l1.location, l2.location) < l2.radius WHERE l1.id = '" + id + "';");
        postgres.query("SELECT l2.id FROM locations AS l1 INNER JOIN locations AS l2 ON ST_Distance(l1.location, l2.location) < l2.radius WHERE l1.id = '" + id + "';", function(error, rows) {
          if (rows) {
            rows.forEach(function(row) {
